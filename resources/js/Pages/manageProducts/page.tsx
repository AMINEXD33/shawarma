...
import { tableElement } from "@/Components/contentTable/contentTable";
...
export default function ManageProducts() {
    ...
    const getCategories = useCallback(async ()=>{
        ...
        if (result.code === "data"){
            ...
            dispatcher(ERROR("can't get categories"));
        }
    }, []);

    // get products
    const getData = useCallback(async ()=>{
        const result = await CustomFetch(route("getProducts"), "GET");
        console.log("called");
        if (result.code==="data"){
            const data_: tableElement[] = result.data?.["products"];
            console.log(data_);
            setDataList(data_);
    ...
    useEffect(()=>{
        ...
        const fs2 = async ()=> await getCategories();
        fs();
        fs2();
    ...
    const editCallback = async (id: number)=>{
        location.assign(route('editPoduct', {product_id: id}))
    }
    ...
    const deleteCallback = async (
        ...
        const result = await CustomFetch(route("deletePro"), "POST", JSON.stringify({
            productId: productId
        }));
        if (result.code==="data"){
            ...
            <div className="actionpanel">
                <div className="grouping">
                    <Slider
                        ...
                        conditionalToggle={safeToToggle}
                    />
                    <p>L’affiche produit est activé</p>
                </div>
                <div className="grouping">
                    <Search
                        name="product"
                    ...
                    <Select
                        ...
                        selected={0}
                        outerState={lang}
                        setOuterState={setLang}
                    />
                    <AddButton
                        withIcon={true}
                        content="nouveau produit"
                        onClickAction={async () => {
                            location.assign(route("addProductPage"));
                        }}
                    />
